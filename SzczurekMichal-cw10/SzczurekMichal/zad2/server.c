...
int add_client(int fd, char* name, struct sockaddr addr){
    for (int i = 0; i < MAX_CLIENTS; i++){
        if (clients[i].in_use != 0 && strcmp(clients[i].name, name) == 0){
            return -1;
        }
...
void del_client(int id){
    ...
    clients_count--;
   	if(id != -1 && clients[opponent_id].in_use != 0){
   		...
   		sendto(clients[opponent_id].fd, "QUIT$", MAX_MESSAGE_LENGTH, 0, &clients[opponent_id].addr, sizeof(struct addrinfo));
...
void set_up_sockets(){
    ...
    bind(local_socket, (struct sockaddr *)&addr, sizeof(struct sockaddr_un));
...
void listner(){
	...
	while(1){
		int sender_fd = check_messages(local_socket, network_socket);
		char buffer[MAX_MESSAGE_LENGTH + 1];
        struct sockaddr sender_addr;
        socklen_t sender_len = sizeof(struct sockaddr);
        recvfrom(sender_fd, buffer, MAX_MESSAGE_LENGTH, 0, &sender_addr, &sender_len);
		...
		if (strcmp(command, "CONNECT") == 0){
            int id = add_client(sender_fd, name, sender_addr);
            if (id == -1){
                sendto(sender_fd, "NAME_TAKEN", MAX_MESSAGE_LENGTH, 0, (const struct sockaddr *)&sender_addr, sizeof(struct addrinfo));
                close(sender_fd);
            }
            else if (clients[id].opponent == -1){
                sendto(sender_fd, "WAITING", MAX_MESSAGE_LENGTH, 0, (const struct sockaddr *)&sender_addr, sizeof(struct addrinfo));
            }
            else{
                int first = rand() % 2;
                int opponent_id = clients[id].opponent;
		        ...
		        sendto(clients[id].fd, message1 ,MAX_MESSAGE_LENGTH, 0, &clients[id].addr, sizeof(struct addrinfo));
		        sendto(clients[opponent_id].fd,message2, MAX_MESSAGE_LENGTH, 0,  &clients[opponent_id].addr, sizeof(struct addrinfo));
            }
        }
            ...
            sendto(clients[clients[id].opponent].fd, message, MAX_MESSAGE_LENGTH, 0, &clients[clients[id].opponent].addr, sizeof(struct addrinfo));
        }
...
void clean(){
	for(int i = 0; i< MAX_CLIENTS; i++){
		if(clients[i].in_use == 1)
		    sendto(clients[i].fd, "QUIT$", MAX_MESSAGE_LENGTH, 0, &clients[i].addr, sizeof(struct addrinfo));  
	}
